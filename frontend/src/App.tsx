...
import { ProjectOutlined, ApiOutlined, SettingOutlined, LogoutOutlined, UserOutlined, PlusOutlined, EnvironmentOutlined, ThunderboltOutlined, PlayCircleOutlined, FileTextOutlined, ClockCircleOutlined, MonitorOutlined } from '@ant-design/icons'
  ...
  return (
    <Card style={{ borderRadius: 16, border: 'none', boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }} styles={{ body: { padding: 0 } }}>
      <div style={{ padding: '24px 24px 0', borderBottom: '1px solid #f0f0f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={() => { setEditing(null); form.resetFields(); setModalOpen(true) }} style={{ borderRadius: 8 }}>
          新建项目
        </Button>
      </div>
      <Table dataSource={projects} columns={columns} rowKey="id" loading={loading} pagination={{ pageSize: 10 }} style={{ margin: '0 24px 24px' }} />
      <Modal title={editing ? '编辑项目' : '新建项目'} open={modalOpen} onOk={handleSave} onCancel={() => setModalOpen(false)} confirmLoading={saving} okButtonProps={{ style: { borderRadius: 8 } }} cancelButtonProps={{ style: { borderRadius: 8 } }}>
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          <Form.Item name="name" label="项目名称" rules={[{ required: true }]}><Input placeholder="输入项目名称" /></Form.Item>
          <Form.Item name="description" label="描述"><Input.TextArea rows={3} placeholder="项目描述（可选）" /></Form.Item>
        </Form>
      </Modal>
    </Card>
  )
...
function App() {
  ...
  const menuItems = [
    ...
    { key: 'console', icon: <MonitorOutlined />, label: '执行控制台' },
  ...
  return (
    <ConfigProvider theme={{ algorithm: theme.defaultAlgorithm, token: appleTokens }}>
      <Layout style={{ minHeight: '100vh', background: '#f5f5f7' }}>
        <Sider width={240} style={{ background: '#ffffff', borderRight: '1px solid #e5e5e5' }}>
          <div style={{ padding: '24px 20px', borderBottom: '1px solid #e5e5e5' }}>
            <div style={{ fontSize: 18, fontWeight: 600, color: '#1d1d1f', letterSpacing: '-0.3px' }}>API Workbench</div>
          </div>
          <Menu mode="inline" selectedKeys={[activeKey]} items={menuItems} onClick={({ key }) => setActiveKey(key)} style={{ background: 'transparent', borderRight: 0, padding: '8px 0' }} />
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '16px 20px', borderTop: '1px solid #e5e5e5', background: '#ffffff' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#f5f5f7', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <Button size="small" icon={<LogoutOutlined />} onClick={() => { logout(); setShowLogin(true) }} style={{ color: '#86868b', borderColor: '#d2d2d7', borderRadius: 8 }} block>
              退出登录
            </Button>
          </div>
        </Sider>
